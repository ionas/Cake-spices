...
* 	'Shout', $this->Profile->Shout->find('count', $this->Paginator->filter('Shout'))
* );
* $list = $this->Paginator->paginate(
* 	'Shout', $this->Profile->Shout->find('all', $this->Paginator->filter('Shout'))
* );
* 
* 
* 
	...
	function paginate($paginateKey, $findResult, $options = array()) {
		if (!isset($this->_runtime[$paginateKey])) {
			...
			$paging = array(
				...
				'pageCount'	=> $pageCount,
				'sort' => $this->options[$paginateKey]['sort'],
				'conditions' => $this->options[$paginateKey]['conditions'],
			);
			// Autoload ExtPaginatorHelper
			$this->C->params['extPaging'][$paginateKey] = $paging;
			$this->C->params['extPaging']['_prefix'] = $this->_settings['prefix'];
			if (!in_array('Paging', $this->C->helpers)
				&& !array_key_exists('Paging', $this->C->helpers)
			) {
				$this->C->helpers[] = 'Paging';
			}
